...
def command_start(update, context: CallbackContext):
    if update.effective_chat.type == "private":
        ...
        update.message.reply_text('Özəl söhbətdə Oyun oynaya bilmərik🧙‍♀️', reply_to_message_id=True, reply_markup=reply_markup)
    else:
        ...
        #update.message.reply_text('Oyun başladı! Hərkəsə uğurlar🥰\n\n🇦🇿'.format(username), reply_to_message_id=True)
...
def command_master(update: Update, context):
    ...
    if not game.is_master_time_left():
        #update.message.reply_text('Aparıcı olmaq üçün {} saniyə qalıb, tələsin 👨‍💻'.format(game.get_master_time_left()),
                                 # reply_to_message_id=True)
        return
...
def is_word_answered(update, context):
    ...
    if game.is_word_answered(user_id, text):
       # update.message.reply_text('*{}* sözünü [{}](tg://user?id={}) tapdı ✅\n\nNövbəti turda o aparıcıdır!'.format(word, username,user_id), reply_to_message_id=True, parse_mode=ParseMode.MARKDOWN)
